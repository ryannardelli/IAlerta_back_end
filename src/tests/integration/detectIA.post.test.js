import detectAIService from "../../services/detectAIContentService";
...
describe("DetectAIContent API - Integration Tests", () => {
  ...
  test("it throw MaxTextLenght if text are much big", async () => {
    await expect(detectAIService("a".repeat(5000))).rejects.toBeInstanceOf(
      MaxTextLenght
    );
  });
  ...
  test("it throw InvalidFormatHug if return is not JSON", async () => {
    ...
    await expect(detectAIService("Hello")).rejects.toBeInstanceOf(
      InvalidFormatHug
    );
  });
});
